...
let classImages = [
    ...
    'https://lh3.googleusercontent.com/drive-viewer/AFGJ81r9UR4XcPV-XO19aElOzEG432RVDJZHFX9T0uD9sN3nTZUYx3wdZRGVQYUB8IT5vQyw9guJHWxC_o3J65tYPnS2zAACfw=s1600',
...
function copyDefaultPath()
{
    const path = document.getElementById('defaultPath');
...
function createCard(container, ttitle, data)
{
    ...
    title.className = 'card-title c_ctitle_text';
...
function createBreakdownTable(val)
{
    let breakdown = document.createElement('div');
    breakdown.className = "c_breakdown_table";
    let row1 = document.createElement('div');
    row1.className = 'row row-cols-4';
    let row2 = document.createElement('div');
    row2.className = 'row row-cols-4';
    ...
    for (s in suffixes)
    {
        // FIXME
        if (s == "3") continue;
        let entry = val[suffixes[s]];
        if (entry)
        {
            let col = document.createElement('div');
            col.className = 'col';
            col.innerText = suffixes[s][0];

            let col2 = document.createElement('div');
            col2.className = 'col';
            col2.innerText = entry;
            ...
            case "MatchesCompleted":
                {
                    for (j in loadDataPrefix[i]){
                        ...
                        let className = i+j;
                        let card = createCard(target, camelCaseSplit(j), val);
                        
                        if (rareStats.includes(className))
            ...
            case "SubClass": // TD K D time
                {
                    for (c in subclasses)
                    {
                        ...
                        let col = createMainCard(container.parentElement, 1+c, subclasses[c][0], classImages[c]);
                        col.appendChild(cardBody);
                        for (r in subclasses[c])
                        {
                            // slice?
                            if (r == "0") continue;
                            ...
                            let className = subclasses[c][r].replace(/\s/g, '');
                            let breakdown = createBreakdownTable(loadDataPrefix['SubClass'][className]);
                            footer.innerText = loadDataPrefix['SubClass'][className]['Playtime'];                            
                            text.appendChild(breakdown);
                            // text.innerHTML = "\
                            // <div class='c_breakdown_table'> \
                            //     <div class='row row-cols-4'> \
                            //         <div class='col'>K</div> \
                            //         <div class='col'>D</div> \
                            //         <div class='col'>KD</div> \
                            //         <div class='col'>T</div> \
                            //     </div> \
                            //     <div class='row row-cols-4'> \
            ...
            case "Weapon": // TK K D
                {
                    for (j in loadDataPrefix[i]){
                        ...
                        let val = loadDataPrefix[i][j];
                        let temp = []
                        let card = createCard(target, camelCaseSplit(j), temp);
                        let text = card.querySelector('.card-text');
                        let breakdown = createBreakdownTable(val);
                        text.appendChild(breakdown);
...
function InitGui()
{
    ...
    for (e in entryNames) {
            ...
            if (entryNames[e].indexOf(prefixes[p], 0) >= 0) {
                ...
                if (suffix == -1)
                {
                    ...
                    if (name.length > prefixS.length)
                        name = name.slice(prefixS.length);
                    loadDataPrefix[prefixS][name] = parsedData[entryNames[e]];
                }
                else
                {
                    // weapon / class name
                    ...
                    loadDataPrefix[prefixS][name][suffixS] = parsedData[entryNames[e]];
                }
        ...
        if (prefix == -1)
            loadDataPrefix['Common'][entryNames[e]] = parsedData[entryNames[e]];
        else {
